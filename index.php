<?php
    ...
    if(isset($_SESSION['library']) && $_SESSION['library'] != ""){
      ...
      $header_key = 1;
     $id = $_SESSION['library'];
    $sql="select * from mls.user where User_id='$id'";
    $result=$conn->query($sql);
    $active_user=$result->fetch_assoc();

    $name=$active_user['Name'];
    $type=$active_user['Type'];
    $balance=$active_user['Account_balance'];
    if($type == "Student"){
        $account = "account/student";
        $account_infor = "Student | Balance R ".$balance;
    }
                
            
      if($active_user['Profile_pic'] == null){
        
      $image = "images/profile-icon.jpg";
      }else{
        $image = $active_user['Profile_pic'];
